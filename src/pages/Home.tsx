...
function ProductCard({ product }: { product: Product }) {
  return (
    <Link to={`/products/${product.id}`} className="group block h-full w-full">
      <article className="flex h-full min-w-0 w-full flex-col overflow-hidden rounded-2xl border border-emerald-100/80 bg-white shadow-md transition duration-500 hover:-translate-y-1.5 hover:shadow-xl">
        ...
        <div className="flex min-w-0 flex-1 flex-col space-y-2 p-3 text-left min-[380px]:p-3.5 sm:space-y-2.5 sm:p-4">
          ...
          <p className="text-[11px] text-slate-500 min-[380px]:text-xs">
            {product.material}
          </p>
...
function ProductCarousel({
  ...
  const updateScrollControls = useCallback(() => {
    ...
    setCanScrollRight(
      hasTrackOverflow && maxScrollLeft - scrollLeft > edgeOffset,
    );
  }, []);
  ...
  const scrollTrack = (direction: number) => {
    ...
    const cardWidth =
      firstCard?.getBoundingClientRect().width ?? track.clientWidth * 0.86;
    ...
    <section className="rounded-3xl border border-white/70 bg-white/85 p-3 shadow-sm backdrop-blur-sm max-[360px]:rounded-2xl max-[360px]:p-2.5 sm:p-6">
      <div className="flex min-w-0 flex-col gap-3 min-[860px]:flex-row min-[860px]:flex-wrap min-[860px]:items-center min-[860px]:justify-between min-[860px]:gap-4">
        <div className="space-y-1">
          <h2 className="text-[15px] font-bold text-slate-900 min-[860px]:text-xl">
            {title}
          </h2>
          <p className="text-xs text-slate-600 min-[860px]:text-sm">
            {description}
          </p>
        </div>
...
export default function Home() {
  ...
  const matchingProducts = useMemo(() => {
    ...
    const filtered = PRODUCTS.filter((product) => {
      ...
      const matchesWeather =
        ...
        selectedWeather.includes(product.weather);
      const matchesPrice =
  ...
  const updateSpecialField = (
    field: keyof SpecialRequestForm,
    value: string,
  ) => {
                    ...
                    <input
                      ...
                      onChange={() =>
                        setSelectedWeather((current) =>
                          toggleSelection(current, weather),
                        )
                      }
                      className="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-300"
                    />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-emerald-100/70 bg-white/85 p-4 shadow-lg backdrop-blur-sm max-[360px]:p-3.5 sm:rounded-[2rem] sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-1">
              <h2 className="text-xl font-extrabold text-slate-900 max-[360px]:text-[17px] sm:text-2xl">
                Specials: Custom Cover
              </h2>
              <p className="text-sm text-slate-600 max-[360px]:text-[13px]">
                Enter your measurements and preferences for a custom special
                cover request.
              </p>
            </div>
          ...
          <form
            ...
            className="mt-5 grid gap-3 sm:mt-6 sm:gap-4 md:grid-cols-2 xl:grid-cols-3"
          >
            <div>
              <label
                htmlFor="special-length"
              ...
              <input
                ...
                onChange={(event) =>
                  updateSpecialField("length", event.target.value)
                }
                placeholder="e.g. 210"
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateSpecialField("height", event.target.value)
                }
                placeholder="e.g. 95"
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 max-[360px]:px-2.5 max-[360px]:py-1.5 max-[360px]:text-[13px]"
                required
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateSpecialField("width", event.target.value)
                }
                placeholder="e.g. 140"
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 max-[360px]:px-2.5 max-[360px]:py-1.5 max-[360px]:text-[13px]"
                required
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(event) =>
                    updateSpecialField("fabric", event.target.value)
                  }
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(event) =>
                    updateSpecialField("color", event.target.value)
                  }
                  className={SPECIAL_SELECT_CLASS}
                >
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(event) =>
                    updateSpecialField("coverOption", event.target.value)
                  }
                  className={SPECIAL_SELECT_CLASS}
                >
            ...
            <div className="xl:col-span-3 flex flex-wrap items-start gap-3 pt-1">
              ...
              <p className="text-xs leading-5 text-slate-500">
                These details can be used to prepare a quote and production
                plan.
              </p>
            </div>
          </form>
          ...
          {specialSubmitted ? (
            <p className="mt-4 break-words rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-800 max-[360px]:px-3 max-[360px]:text-[13px]">
              Special request captured for {specialRequest.coverOption}:{" "}
              {specialRequest.length} x {specialRequest.width} x{" "}
              {specialRequest.height} cm. <br />A representative will be in
